...
Проект для передачи файлов по локальной сети меж `Producer` и `Consumer` с использованием TCP-соединения. Реализована проверка целостности данных с помощью хеширования (SHA256) и подтверждение получения пакетов (ACK/NACK).
...
- Поддержка получения данных от множества `Producer` на 1 `Consumer`
